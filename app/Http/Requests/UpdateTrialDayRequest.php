...
class UpdateTrialDayRequest extends FormRequest
{
    public function authorize()
    {
        return true; // Ubah ini berdasarkan logika otorisasi Anda
    }

    public function rules()
    {
        $trialDayId = $this->route('data_trial');

        return [
            'full_name' => 'required|string|max:255',
            'email' => 'required|email|max:255|unique:trial_days,email,' . $trialDayId,
